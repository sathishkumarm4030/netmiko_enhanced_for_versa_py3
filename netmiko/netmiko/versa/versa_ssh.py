...
class VersaSSH(BaseConnection):
    """
    Implement methods for interacting with Versa Networks devices.

    Disables `enable()` and `check_enable_mode()`
    methods.  Overrides several methods for Versa-specific compatibility.
    """
    ...
    def session_preparation(self):
        ...
        self.set_terminal_width(command='set screen width 0')
    ...
    def _return_cli(self):
        """Return to the Versa CLI."""
        return self.send_command("exit", expect_string=r"[#>]")

    def _disable_complete_on_space(self):
        """
        Versa tries to auto complete commands when you type a "space" character.
    ...
    def send_command_expect(self, *args, **kwargs):
        ...
        :type kwargs: dict
        """
        if 'cmd_verify' not in kwargs:
            kwargs['cmd_verify'] = False
        if 'strip_prompt' not in kwargs:
            kwargs['strip_prompt'] = False
        if 'strip_command' not in kwargs:
            kwargs['strip_command'] = False
        return super().send_command_expect(*args, **kwargs)
    ...
    def commit(
        ...
        if "No modifications to commit." in output:
            return output
    ...
    def strip_context_items(self, a_string):
        """Strip Versa-specific output.

        Versa will also put a configuration context:
        [edit]
...
class VersaFileTransfer(BaseFileTransfer):
    """Versa SCP File Transfer driver."""
